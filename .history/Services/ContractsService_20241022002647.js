...
exports.getContracts = expressAsyncHandler(async (req, res) => {
  ...
  // افترض أن كل عقد لديه حقل 'notifications' يحتوي على قائمة بالتواريخ
  const now = new Date();

  // قم بتصفية الإشعارات بناءً على التاريخ
  const notificationsToSend = getDoc.flatMap(contract => 
    contract.notifications.filter(notification => {
      const notificationDate = new Date(notification.date);
      const diffTime = Math.abs(now - notificationDate);
      const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
      return diffDays > 2; // تحقق إذا كان الفرق أكبر من يومين
    }).map(notification => ({
  ...
  notificationsToSend.forEach(({ contractId, notification }) => {
    // إنشاء إشعار لكل ماركتر
    const marketersNotifications = notification.marketers.map((marketer) => {
      if (req.body.kind !== "taking") {
        return createNotificationsModel.create({
          assignedBy: notification.assignedBy, // استخدم المعطيات الصحيحة
          assignedTo: marketer._id,
          contracts: contractId,
          msg: `${req.user.name} قام بإرسال إشعار عن ${notification.attorney}`, // تعديل الرسالة
        });
      }
    ...
    const managersNotifications = employees.map((manager) => {
      if (req.body.kind !== "taking") {
        return createNotificationsModel.create({
          ...
          contracts: contractId,
          msg: `${req.user.name} قام بإرسال إشعار عن ${notification.attorney}`,
        });
      }
    }).filter(Boolean); // تصفية القيم الفارغة

    // دمج الإشعارات معاً (المسوقين والمدراء)
    allNotifications.push(...marketersNotifications, ...managersNotifications);
  });
