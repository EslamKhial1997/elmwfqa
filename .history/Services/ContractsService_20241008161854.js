...
exports.getContracts = expressAsyncHandler(async (req, res, next) => {
  expressAsyncHandler(async (req, res) => {
    let fillter = {req.user.role === "manager" ? {} : { employees: req.user._id };};
    if (req.filterObject) {
      fillter = req.filterObject;
    }

    const countDocs = await createContractsModel.countDocuments();
    const ApiFeatures = new FeatureApi(createContractsModel.find(fillter), req.query)
      .Fillter(Model)
      .Sort()
      .Fields()
      .Search(keyword)
  ...
  // بناء الاستعلام
  // const query = req.user.role === "manager" ? {} : { employees: req.user._id };

  // // الحصول على العقود
  // const contracts = await createContractsModel.find(query);
  // // إرجاع النتيجة
  // res.status(200).json({
  //   status: "success",
  //   data: contracts,
