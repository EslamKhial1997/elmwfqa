...
exports.getUsers = expressAsyncHandler(async (req, res, next) => {
  ...
  // إذا كان المستخدم ليس "manager"، قم بتحديد الفلتر للتحقق من وجوده في marketers array
  if (req.user.role !== "manager") {
    filter = { marketers: { $in: [req.user._id] } }; // التأكد من أن معرف المستخدم موجود في مصفوفة marketers
  }
