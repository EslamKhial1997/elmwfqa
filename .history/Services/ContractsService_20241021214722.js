...
exports.updateContractsStatus = expressAsyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    try {
      if (
        req.body.statusDetails.emptying.type === "emptying" &&
        req.body.contractsStatus === "emptying"
      ) {
        const document = await createDocumentsModel.findOne({contracts: req.body.id})
        console.log(document);
        if (document.amount >req.body.saay ) {
          return   res.status(500).json({ status: "error", msg: "Failed to update contract" });
        }
