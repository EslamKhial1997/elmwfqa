...
exports.createDouments = expressAsyncHandler(async (req, res) => {
  try {
    ...
    console.log(req.body.attorney._id , contracts._id.toString());

    const result = await createContractsModel.updateOne(
      { _id: contracts._id.toString() }, // تحقق من الـ ID للمستند الذي تريد تحديثه
      { $pull: { attorney: { _id: req.body.attorney._id } } } // حذف الاسم
    );
    console.log(result);
