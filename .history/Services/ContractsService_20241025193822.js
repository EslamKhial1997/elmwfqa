...
exports.createContracts = expressAsyncHandler(async (req, res) => {
  ...
  req.body.code =   Math.floor(1000 + Math.random() * 9000)// Generates a 4-digit random number
...
exports.getContracts = expressAsyncHandler(async (req, res) => {
  ...
  notificationsToSend.forEach(async (notification) => {
    if (
      !notification.depositRally &&
      notification.deposit &&
      notification.available
    ) {
      return;
    }
    const notificationDoc = await createNotificationsModel.find({
