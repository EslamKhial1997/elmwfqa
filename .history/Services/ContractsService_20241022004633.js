...
exports.getContracts = expressAsyncHandler(async (req, res) => {
  ...
  // الحصول على الإشعارات التي تحتاج إلى إرسال
  const notificationsToSend = getDoc.filter((notification) => {
    const notificationDate = new Date(notification.createdAt);
    const diffTime = Math.abs(now - notificationDate);
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    return diffDays >= 2 && !notification.notificationSent; // تحقق إذا كان الفرق أكبر من يومين وأيضًا لم يتم إرسال الإشعار
  });

  // إذا لم يكن هناك إشعارات لإرسالها
  ...
  for (const notification of notificationsToSend) {
    const employees = await createEmployeesModel.find({
      $or: [
        { role: "manager" },
        { _id: notification.employees._id },
      ],
    });

    const marketersNotifications = employees.map(async (marketer) => {
      return createNotificationsModel.create({
        assignedBy: notification.user._id,
        assignedTo: marketer._id,
        contracts: notification._id,
        msg: `${notification.user.name} تم انشاء العقد منذ يومين`, // الرسالة
      });
    });

    // إرسال جميع الإشعارات دفعة واحدة
    await Promise.all(marketersNotifications);

    // تحديث العقد للإشارة إلى أن الإشعار قد تم إرساله
    await createContractsModel.findByIdAndUpdate(notification._id, {
      notificationSent: true, // أو أي حقل آخر تشير إلى أنه تم إرسال الإشعار
    });
  }

  res.status(201).json({ results: getDoc.length, PaginateResult, data: getDoc });
});
