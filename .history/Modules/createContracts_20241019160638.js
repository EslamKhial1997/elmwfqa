...
const ImageURL = (doc) => {
  if (doc.pdf && !doc.pdf.startsWith(`${process.env.BASE_URL}/contracts`)) {
    doc.pdf = `${process.env.BASE_URL}/contracts/${doc.pdf}`;
  }
};
...
exports.resizeImage = espr(async (req, res, next) => {
  if (req.file) {
    req.body.pdf = req.file.filename; // حفظ اسم الملف فقط (المسار النسبي)
  }
  next();
});
